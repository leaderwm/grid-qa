...
LLM_USER_OBSERVER_EVENTS = Counter(
    "grid_llm_user_observer_events_total", "LLM-as-a-User 语义事件", ["result"]
)
LLM_USER_OBSERVER_EXPORT = Counter(
    "grid_llm_user_observer_export_total", "LLM-as-a-User OTLP 批量导出", ["result"]
)
...
# 知识自进化回流增益（B5）：lift = after - before；正=回流有效，<=0=无效下调 quality_score
EVOLUTION_LIFT = Histogram(
    "grid_evolution_lift",
    "自进化回流增益(正=有效)",
    buckets=(-1.0, -0.2, 0, 0.2, 0.5, 1.0),
)
