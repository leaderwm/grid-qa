...
# 自适应 prompt 已迁移到 knowledge_backbone（供两套服务共用，消除重复）
from app.services.knowledge_backbone import classify_query, generate_adaptive_draft  # noqa: F401
...
async def ai_draft(gap_id: int, model_type: str | None = None) -> str:
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            # 2. backbone 自适应草稿生成（问题分类 + gap 上下文注入）
            draft = await generate_adaptive_draft(
                row.query, contexts,
